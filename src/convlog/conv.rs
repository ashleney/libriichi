...
fn tenhou_kyoku_to_mjai_events(kyoku: &Kyoku) -> Result<Vec<Event>> {
    ...
    let attempt = |backtracks: &mut AHashMap<Tile, BackTrack>| -> Result<Vec<Event>> {
        ...
        events.push(Event::StartKyoku {
            ...
            dora_marker: dora_feed.next().context("insufficient dora indicators")?,
            oya,
        ...
        loop {
            ...
            let take = take_events[actor].get(take_idxs[actor]).context("insufficient takes")?;
            take_idxs[actor] += 1;

            if let Some((target, pai)) = take.naki_info() {
                if pai != last_discard || last_actor.is_some_and(|a| a != target || a == actor as u8) {
                    bail!("unexpected naki");
                }
            }
            ...
            actor = (0..4)
                ...
                .and_then(|a| {
                    ...
                    let has_same_dahai_in_future = discard_sets[actor].get(&last_discard).is_some_and(|&v| v > 0);
...
fn finalize_discards(takes: &[Event], discards: &mut Vec<Event>) {
    ...
    for take in takes {
        ...
        if let Event::Dahai { pai, tsumogiri, actor } = discards[di] {
            if tsumogiri {
...
fn take_action_to_events(actor: u8, takes: &[ActionItem]) -> Result<Vec<Event>> {
    takes
        .iter()
        .map(|take| match take {
            ActionItem::Tsumogiri(_) => Err(anyhow!("unexpected tsumogiri")),
            ...
            ActionItem::Naki(naki_string) => {
                ...
                if naki.contains(&b'c') {
                    ...
                    Ok(Event::Chi {
                        ...
                        consumed: [tiles_from_tenhou_bytes(&naki[3..5])?, tiles_from_tenhou_bytes(&naki[5..7])?],
                    })
                    ...
                    match idx {
                        ...
                        2 => Ok(Event::Pon {
                            ...
                            consumed: [tiles_from_tenhou_bytes(&naki[0..2])?, tiles_from_tenhou_bytes(&naki[5..7])?],
                        }),
                        ...
                        4 => Ok(Event::Pon {
                            ...
                            consumed: [tiles_from_tenhou_bytes(&naki[0..2])?, tiles_from_tenhou_bytes(&naki[2..4])?],
                        }),

                        // ???
                        _ => bail!("invalid naki {}", naki_string),
                    }
...
pub fn tiles_from_tenhou_bytes(b: &[u8]) -> Result<Tile> {
    ...
    let id: u8 = s.parse().context(anyhow!("invalid tile {}", s.clone().into_owned()))?;

    let tenhou_tile = TenhouTile::try_from(id).context(anyhow!("invalid tile {}", s.clone().into_owned()))?;
