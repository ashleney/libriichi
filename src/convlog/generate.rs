...
pub fn generate_mjai_logs(board: Board) -> Result<Vec<Event>> {
    ...
    let mut kakan_candidates = [false; 34]; // kakan is the only event that does not have a completely absolute order
    loop {
        ...
        let next_event_targetting_actor = fuuro_events_iter_abs
            .iter_mut()
            .filter_map(|it| it.next_if(|event| event.naki_info().is_some_and(|(target, _)| target == current_actor)))
            .max_by_key(|event| event.naki_to_ord());
        let sutehai;
...
mod test {
    ...
    fn test_generate_logs() -> Result<()> {
        ...
        println!("{}", tenhou.to_string_pretty()?);
        assert_eq!(
